...
public interface NoticeRepository extends JpaRepository<Notice, Long> {
	...
	@Transactional
	@Query(value = "select * from notice where ((now() between start_time and end_time or (extract(year from time)=:year and extract(month from time)=:month and extract(day from time)=:day))) and channel_id = (select id from channel where token = :channelId)", nativeQuery = true)
